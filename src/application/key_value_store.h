...
public:
  ...
  KeyValueStore(size_t home_node) {
    this->home_node = home_node;
  }

  /**
   * Puts the dataframe to the associated key. If the key-value store is not the one
   * that is supposed to store this dataframe, it will give it to the correct home node.
  ...
  void put(Key *key, DataFrame *value) {
    ...
    if (key->home_id == this->home_node) {
      ...
      // TODO: Link the key value stores together so they can talk to each other.
    }
  }
   ...
   * @return The dataframe value associated with the key. The dataframe will be dynamically allocated
   *         the caller will now own the dataframe.
   */
  DataFrame *waitAndGet(Key &key) {
    if (key.home_id == this->home_node) {
      ...
      // TODO: Create a rower/fielder to copy the dataframe into a new dataframe.
  ...
  /**
   * Generates a Dataframe value, and then stores it within the map at the specified key.
   * @param key The key associated with the new dataframe.
